...
def fetch_today_cw_review_msgs(token1: str, token2: str, target_date: datetime,
    ...
    # DMルーム名を収集（ルーム一覧のtypeフィールドで判定）
    dm_room_names: set = set()
    for r in rooms1:
        if r.get('type') == 'direct':
            dm_room_names.add(r.get('name') or f'room_{r.get("room_id","")}')
    ...
    for r in target_rooms2:
        if r.get('type') == 'direct':
            dm_room_names.add(r.get('name') or f'room_{r.get("room_id","")}')
    print(f'  → {len(target_rooms2)}室（フィルタ後）並列取得開始 / DM室: {len(dm_room_names)}室')
    ...
    return result, total_rooms, dm_room_names
...
def build_cw_review(raw_msgs_by_room: dict, month_str: str,
                    yesterday_date: datetime = None, total_rooms: int = 0,
                    dm_room_names: set = None) -> dict:
    ...
    _dm_rooms = dm_room_names or set()
...
# ============================================================
# ⑦ メンタルマネジメント分析（INFJ-A特性ベース）
# ============================================================
MBTI_PROFILE = """
【MBTIタイプ】INFJ-A（提唱者 / Advocate）
【特性】
- 内向型(I): 深い人間関係を好む。大量の対人コミュニケーションで消耗しやすく、一人の内省時間でエネルギーを回復する
- 直感型(N): 大局・ビジョン・本質を重視。意味のある目標達成に強く動機づけられる
- 感情型(F): 深い共感力と強い倫理観。他者の苦しみを自分のことのように感じ、抱え込みやすい
- 判断型(J): 計画・秩序・完結を好む。未解決のまま放置される状況や方向性のズレにストレスを感じる
- 主張型(A): 自己肯定感が比較的安定しているが、内面の理想と現実のギャップには敏感
【バーンアウトリスクサイン】
- 深夜・早朝（22時以降、6時以前）の発言増加（内向型の「ひとり作業」モードへの逃避）
- 1日の発言数が急減（理想と現実のギャップ消耗による引きこもり傾向）
- 同じ問題に繰り返し対処しているルームの出現（ビジョンと現場のズレによる疲弊）
- カレンダーの予定が連日続き内省時間がない
【回復のヒント】
- 一人で静かに考える時間（内省・読書・戦略思考）を意識的に確保する
- 「自分のビジョンが誰かの役に立っている」実感が最大の充電源になる
- 完璧主義を手放し「今日できたことを認める」習慣をつける
- 信頼できる少数の人との深い対話が回復を加速させる
"""

def analyze_mental_management(cw_review: dict, calendar_data: dict,
                               month_str: str) -> dict:
    """CWログ・カレンダーデータからINFJ-Aメンタルマネジメント分析を実行"""
    client = anthropic.Anthropic(api_key=CLAUDE_API_KEY)
    ...
    prompt = f"""あなたはメンタルヘルスコーチです。以下のデータをもとに、INFJ-A型経営者のメンタルマネジメントレポートをJSONで作成してください。
...
{{
  ...
  "todayAdvice": ["今日意識すること1（INFJ-Aの特性に沿った具体的アドバイス）", "2", "3"],
...
def main():
    ...
    all_room_msgs, total_cw_rooms, dm_room_names = fetch_today_cw_review_msgs(CW_TOKEN_1, CW_TOKEN_2, review_date, days=2)
    ...
    cw_review = build_cw_review(all_room_msgs, month_str, yesterday_date=review_date,
                                total_rooms=total_cw_rooms, dm_room_names=dm_room_names)
